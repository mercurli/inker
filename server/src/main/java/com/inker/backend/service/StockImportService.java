...
public class StockImportService {
    ...
    private final StockConceptProvider stockConceptProvider;
    private final StockRepository stockRepository;
    private final StockLogoService stockLogoService;

    public StockImportService(StockProvider stockProvider,
                              StockConceptProvider stockConceptProvider,
                              StockRepository stockRepository,
                              StockLogoService stockLogoService) {
        this.stockProvider = stockProvider;
        this.stockConceptProvider = stockConceptProvider;
    ...
    public ImportResultDto importAStocksExcludeStAndBse() {
        ...
        for (StockProvider.ProviderStock candidate : candidates) {
            ...
            if (!stock.isConceptsManuallyEdited()) {
                stock.setConcepts(stockConceptProvider.fetchConcepts(candidate.code(), candidate.market()));
            }
            ...
            if (stock.getLogo() == null || stock.getLogo().isBlank()) {
                stockLogoService.downloadLogo(candidate.code(), null).ifPresent(stock::setLogo);
            }
            stock.setSt(false);
            toSave.add(stock);
        }
