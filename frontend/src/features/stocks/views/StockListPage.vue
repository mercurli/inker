        ...
        <table class="stock-table">
          ...
          <thead>
            <tr>
              ...
              <th class="th-sortable" @click="onSort('code')">名称 / 代码{{ store.sortIndicator('code') }}</th>
              <th class="th-sortable" @click="onSort('latestPrice')">最新价{{ store.sortIndicator('latestPrice')
              }}</th>
              <th class="th-sortable" @click="onSort('changePercent')">涨跌幅{{
                store.sortIndicator('changePercent') }}</th>
              <th class="th-sortable" @click="onSort('amount')">成交额{{ store.sortIndicator('amount') }}</th>
              <th class="th-sortable" @click="onSort('turnoverRate')">换手{{ store.sortIndicator('turnoverRate') }}</th>
              <th class="th-sortable" @click="onSort('totalMarketValue')">总市值{{
                store.sortIndicator('totalMarketValue') }}</th>
              <th class="th-sortable" @click="onSort('dynamicPeRatio')">市盈率{{ store.sortIndicator('dynamicPeRatio') }}</th>
              <th class="th-sortable" @click="onSort('industry')">行业{{ store.sortIndicator('industry') }}</th>
              <th class="th-center">概念</th>
              <th class="th-sortable th-center" @click="onSort('boardType')">板块{{ store.sortIndicator('boardType') }}</th>
          ...
          <tbody>
            ...
            <tr v-for="(stock, index) in store.stocks.value" :key="stock.id"
              ...
              <td >{{ store.formatPrice(stock.latestPrice) }}</td>
              <td >
                <PriceChangeChip :tone="store.changeClass(stock.changePercent)"
                  :value="store.formatPercent(stock.changePercent)" />
              </td>
              <td >{{ store.formatAmount(stock.amount) }}</td>
              <td >{{ store.formatTurnoverRate(stock.turnoverRate) }}</td>
              <td >{{ store.formatMarketValue(stock.totalMarketValue) }}</td>
              <td >{{ store.formatPeRatio(stock.dynamicPeRatio) }}</td>
              <!-- 行业 -->
